...
@interface FBNearbyController ()<WKNavigationDelegate>
...
//        FBHomeConfItemModel *first = [[FBHomeConfManager shareInstance].homeConfModel.menu_button firstObject];
        ...
        if([last.shortcut_type isEqualToString:@"2"]){//网页
            self.webView.hidden = NO;
            //判断展示网页还是模版
            if(!self.webLoadSComplete){
                [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:last.shortcut_value]]];
...
- (void)sureBtnClick
{
    if([FBHomeConfManager shareInstance].templateModel.template_page_2.is_login == 1){//需要登录
        ...
        webvc.navTitle = @"";
        webvc.urlStr = [FBHomeConfManager shareInstance].templateModel.template_page_2.url;
